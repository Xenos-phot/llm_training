...
def test_model(product_name, product_description, product_price, layout, image_url, model=None, tokenizer=None, product_color=None, fontFamilyList=None, logger=None):
    ...
    try:
        ...
        if logger is None:
            ...
            logger.info("Layout template not found, using default")
    ...
    if product_color is None or fontFamilyList is None:
        if logger is None:
            print("Generating color palette and font family list from image...")
        else:
            logger.info("Generating color palette and font family list from image...")
        ...
        if logger is None:
            ...
            logger.info(f"Generated color palette: {product_color[:100] if product_color else 'None'}...")
            logger.info(f"Generated font family list: {fontFamilyList}")
    ...
    if logger is None:
        ...
        logger.info("-" * 50)
        logger.info(input_text[:500] + "..." if len(input_text) > 500 else input_text)
        logger.info("-" * 50)
    ...
    if logger is None:
        ...
        logger.info(f"Time taken to generate banner: {time.time() - generate_time} seconds")
    ...
    try:
        ...
        if logger is None:
            ...
            logger.info(f"Error post processing: {e}")
